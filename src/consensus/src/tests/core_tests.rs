...
async fn handle_proposal() {
    ...
    let expected =
        bincode::serde::encode_to_vec(&ConsensusMessage::Vote(vote), bincode::config::standard())
            .unwrap();

    // Run a core instance.
    let (tx_core, _rx_proposer, _rx_commit) = core(public_key, secret_key, committee.clone());
...
async fn generate_proposal() {
    ...
    // Run a core instance.
    let (tx_core, mut rx_proposer, _rx_commit) = core(next_leader, next_leader_key, committee());
...
async fn commit_block() {
    ...
    // Run a core instance.
    let (public_key, secret_key) = keys().pop().unwrap();
    let (tx_core, mut rx_proposer, mut rx_commit) = core(public_key, secret_key, committee());
...
async fn local_timeout_round() {
    ...
    // Run a core instance.
    let (_tx_core, _rx_proposer, _rx_commit) = core(public_key, secret_key, committee.clone());
