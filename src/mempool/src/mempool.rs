...
type Store = store::Store<Vec<u8>, Vec<u8>>;
pub struct Mempool {
...
impl MessageHandler for MempoolReceiverHandler {
    async fn dispatch(&self, writer: &mut Writer, serialized: Bytes) -> Result<(), Box<dyn Error>> {
        ...
        let serialized_slice: Vec<u8> = serialized.into();
        // Deserialize and parse the message.
        match bincode::serde::decode_from_slice(&serialized_slice, bincode::config::standard()) {
            Ok((MempoolMessage::Batch(..), _)) => self
                .tx_processor
                .send(serialized_slice)
                .await
                .expect("Failed to send batch"),
            Ok((MempoolMessage::BatchRequest(missing, requestor), _)) => self
                .tx_helper
